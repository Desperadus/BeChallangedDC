...
async def befriend(message):
    with cnx.cursor() as cursor:
        ...
        #Query
        cursor.execute("SELECT friends_channel_id FROM users WHERE id = %s", (user.id,))
        ...
        cursor.execute("SELECT friends_channel_id FROM users WHERE id = %s", (message.author.id,))
        result = cursor.fetchone()
        if result is not None:
...
async def post_image(message, image_url, msgdescription = ""):
    ...
    with cnx.cursor() as cursor:
        cursor.execute("SELECT feed_channel_id FROM users INNER JOIN isfriends ON users.id = isfriends.user2 WHERE isfriends.user1 = %s", (message.author.id,))
